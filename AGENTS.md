...
| `DB_SSL_MODE` | `require` | **`require`** | 生产禁止disable |
...
| `SMTP_HOST` | `smtp.qiye.aliyun.com` | 按需 | 邮件服务 |
| `SMTP_PORT` | `465` | `465` | |
| `SMTP_USER` | 按需 | 按需 | |
| `SMTP_PASSWORD` | 按需 | **必须设置** | |
| `SMTP_FROM` | 按需 | 按需 | |
| `E2E_ADMIN_EMAIL` | `system@eninte.com` | - | 测试管理员邮箱 |
...
- ❌ 禁止因"功能未实现"而跳过测试 → **必须先实现功能，再运行测试**
- ❌ 禁止因"环境问题"而跳过测试 → **必须修复环境配置**
- ❌ 禁止因"端点未实现"而跳过测试 → **必须实现端点**
- ❌ 禁止使用过于宽松的断言（如 `assert.True(t, code >= 400)`）→ **必须使用精确断言**
...
# 或手动运行
E2E_ADMIN_EMAIL="system@eninte.com" E2E_ADMIN_PASSWORD="Admin123!" go test -v -tags=e2e ./test/e2e/...
```

E2E测试需要：
1. 服务运行在 `http://localhost:9090`
2. 数据库连接可用
3. 管理员账户已配置

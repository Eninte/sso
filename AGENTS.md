...
# 2. 配置环境变量（复制.env.example到.env.test）
...
| 变量 | 测试环境 | 生产环境 | 说明 |
|------|---------|---------|------|
| `DB_HOST` | `192.168.1.3` | 按需 | 数据库主机 |
| `DB_PORT` | `5432` | `5432` | 数据库端口 |
| `DB_NAME` | `sso_test` | `sso` | 数据库名称 |
| `DB_USER` | `sso` | `sso` | 数据库用户 |
| `DB_PASSWORD` | `sso` | **强密码** | ⚠️ 生产环境必须使用强密码 |
| `DB_SSL_MODE` | `require` | **`require`** | ⚠️ 生产环境禁止disable |
...
| 变量 | 测试环境 | 生产环境 | 说明 |
|------|---------|---------|------|
| `BCRYPT_COST` | `10` | **`>=12`** | 密码哈希强度 |
| `JWT_PRIVATE_KEY_PATH` | `./keys/private.pem` | `./keys/private.pem` | 运行 `make generate-keys` 生成 |
| `JWT_PUBLIC_KEY_PATH` | `./keys/public.pem` | `./keys/public.pem` | |
| `CORS_ALLOWED_ORIGINS` | `http://localhost:3000` | **`https://your.com`** | 允许的跨域源 |

### SMTP配置（邮件服务）

| 变量 | 测试环境 | 生产环境 |
|------|---------|---------|
| `SMTP_HOST` | `smtp.qiye.aliyun.com` | 按需 |
| `SMTP_PORT` | `465` | `465` |
| `SMTP_USER` | 按需 | 按需 |
| `SMTP_PASSWORD` | 按需 | **必须设置** |
| `SMTP_FROM` | 按需 | 按需 |
